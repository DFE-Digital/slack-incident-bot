SlackRubyBotServer::Events.configure do |config|
  config.on :action, 'view_submission' do |action|
    ...
    threads = []
    threads << Thread.new { client.conversations_invite(channel: channel_name, users: "#{incident_comms_lead},#{incident_tech_lead},#{incident_support_lead}") }
    threads << Thread.new { client.conversations_setTopic(channel: channel_name, topic: "Description: #{incident_description.capitalize}\n Priority: #{incident_priority}\n Comms lead: <@#{incident_comms_lead}>\n Tech lead: <@#{incident_tech_lead}>\n Support lead: <@#{incident_support_lead}>") }
    threads << Thread.new { client.chat_postMessage(channel: "general", text: ":rotating_light: <!here> A new incident has been opened :rotating_light:\n> *Title:* #{incident_title.capitalize} \n>*Priority:* #{incident_priority}\n>*Comms:* <##{channel_name}>") }

    message = client.chat_postMessage(channel: channel_name, text: "Welcome to the incident channel. Please review the following docs:\n> <https://docs.google.com/document/d/1QN72Gom-M6bysVqC8Enq0crOqE_cFT82tBfMy1djF6s/edit#heading=h.pu6me2z5fcxt|Incident playbook> \n><https://docs.google.com/document/d/1uUeoUiXejyB5XnVjWniDvX3nzoxdRj1I/edit|Incident categorisation>")
